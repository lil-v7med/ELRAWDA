...
import { authMiddleware, JWT_SECRET, AuthenticatedRequest } from '../middleware/auth.js';
...
// Log audit helper
async function logAudit(userId: number | null, action: string, req: AuthenticatedRequest) {
...
router.get('/backup', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;
...
// 9. DELETE ACCOUNT
router.post('/delete-account', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;
...
router.post('/verify-reset-code', async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.post('/resend-reset-code', async (req: AuthenticatedRequest, res: Response) => {
  try {
