...
router.get('/summary', authMiddleware, async (req, res) => {
    ...
    `, [userId]);
        ...
        if (emergCoverageMonths >= 6.0)
            emergRiskLevel = 'منخفض';
        else if (emergCoverageMonths >= 3.0)
            emergRiskLevel = 'متوسط';
        ...
        if (savingsRatio >= 25) {
            ...
            savingsRateStatus = 'ممتاز';
        }
        else if (savingsRatio >= 15) {
            savingsRateScore = 85;
            savingsRateStatus = 'جيد جداً';
        }
        else if (savingsRatio >= 5) {
            savingsRateScore = 60;
            savingsRateStatus = 'جيد';
        }
        ...
        if (debtRatioVal > 50) {
            debtRatioScore = 30;
            debtRatioStatus = 'يحتاج تحسين';
        }
        else if (debtRatioVal > 30) {
            debtRatioScore = 60;
            debtRatioStatus = 'جيد';
        }
        else if (debtRatioVal > 10) {
            debtRatioScore = 85;
            debtRatioStatus = 'جيد جداً';
        }
        ...
        if (totalBudgetLimit > 0) {
            ...
            if (ratio <= 0.8) {
                ...
                expenseControlStatus = 'ممتاز';
            }
            else if (ratio <= 1.0) {
                expenseControlScore = 85;
                expenseControlStatus = 'جيد جداً';
            }
            else if (ratio <= 1.25) {
                expenseControlScore = 60;
                expenseControlStatus = 'جيد';
            }
            else {
                expenseControlScore = 30;
                expenseControlStatus = 'يحتاج تحسين';
            }
        }
        ...
        if (recurringCount >= 2) {
            ...
            incomeStabilityStatus = 'ممتاز';
        }
        else if (recurringCount === 1) {
            incomeStabilityScore = 85;
            incomeStabilityStatus = 'جيد جداً';
        }
        ...
        if (emergCoverageMonths >= 6.0) {
            ...
            emergencyFundStatus = 'ممتاز';
        }
        else if (emergCoverageMonths >= 3.0) {
            emergencyFundScore = 80;
            emergencyFundStatus = 'جيد جداً';
        }
        else if (emergCoverageMonths >= 1.0) {
            emergencyFundScore = 60;
            emergencyFundStatus = 'جيد';
        }
        ...
        if (assetTypesCount >= 3) {
            investmentScore = 100;
            investmentStatus = 'ممتاز';
        }
        else if (assetTypesCount === 2) {
            investmentScore = 75;
            investmentStatus = 'جيد';
        }
        ...
        // Aggregate Score
        const finalHealthScore = Math.round((savingsRateScore * 0.20) +
            (debtRatioScore * 0.15) +
